...
def expander_model_parameters(
    LLM_provider="Groq",
    text_input_API_key="Groq API Key",
    list_models=["gemma2-9b-it", "llama3-8b-8192", "llama3-70b-8192", "mixtral-8x7b-32768"],
):
...
def sidebar_and_documentChooser():
    ...
    with st.sidebar:
        st.caption(
            "🚀 A retrieval augmented generation chatbot powered by 🔗 Langchain, Groq, Google Generative AI and 🤗"
        )
        st.write("")
    ...
    with tab_open_vectorstore:
        ...
        if clicked:
            ...
            error_messages = []
            if (
                not st.session_state.groq_api_key
...
def select_embeddings_model():
    """Select embeddings models: HuggingFaceEmbeddings or GoogleGenerativeAIEmbeddings."""
    if st.session_state.LLM_provider == "Groq":
        # Using a local model for embeddings to avoid API dependency
        with st.spinner("Loading local embedding model (first time may take a while)..."):
...
def create_retriever(
    ...
    Parameters:
        ...
        cohere_model (str): model used by Cohere, e.g. "rerank-multilingual-v3.0"
...
def CohereRerank_retriever(
    ...
    Parameters:
       ...
       cohere_model: the Cohere model, default = "rerank-multilingual-v3.0"
                ...
                if st.session_state.uploaded_file_list is not None:
                    ...
                    try:
                        ...
                        if str(e) == "'0'" or str(e) == "0":
                            err_msg = "Failed to create vectorstore: The API returned an error such as rate limit, model loading, or invalid API key."
                            if hasattr(embeddings, 'embed_documents'):
                                try:
                                    res = embeddings.embed_documents(["test"])
                                    if isinstance(res, dict) and "error" in res:
                                        err_msg = f"HuggingFace API Error: {res['error']}"
                                except Exception:
                                    pass
                            st.error(err_msg)
                        else:
...
def create_memory(model_name="llama3-8b-8192", memory_max_token=None):
    """Creates a ConversationSummaryBufferMemory for Groq and Google"""
...
Standalone question:""",
    ...
    if st.session_state.LLM_provider == "Groq":
        standalone_query_generation_llm = ChatGroq(
            api_key=st.session_state.groq_api_key,
            model=st.session_state.selected_model,
            temperature=0.1,
        )
        response_generation_llm = ChatGroq(
            api_key=st.session_state.groq_api_key,
            model=st.session_state.selected_model,
...
def get_response_from_LLM(prompt):
    ...
    try:
        ...
        if st.session_state.LLM_provider == "Groq":
            pass # Currently ChatGroq does not output answer with "\nAnswer:" like HuggingFace did
